...
{{-- Nội dung chính của trang (phần biểu đồ) --}}
@section('content')
            ...
            <div class="row">
                <div class="col-lg-8">
                    <div style="width:100%; height:70vh;">
                        <div
                            style="background:#fff; padding:0; border-radius:6px; height:100%; box-shadow: 0 0 0 1px rgba(0,0,0,0.02); display:flex; flex-direction:column; overflow:hidden;">
                            <div style="padding:16px;">
                                <div class="d-flex align-items-center justify-content-between">
                                    <h5 class="m-0">Báo cáo doanh thu</h5>
                                    <div>
                                        <div class="btn-group" role="group" aria-label="Quick select">
                                            <input type="radio" class="btn-check" name="quickSelect" id="qs-weeks"
                                                autocomplete="off">
                                            <label class="btn btn-outline-secondary" for="qs-weeks"
                                                data-mode="last_weeks"> Tuần</label>

                                            <input type="radio" class="btn-check" name="quickSelect" id="qs-months"
                                                autocomplete="off" checked>
                                            <label class="btn btn-outline-secondary" for="qs-months"
                                                data-mode="last_months"> Tháng</label>

                                            <input type="radio" class="btn-check" name="quickSelect" id="qs-years"
                                                autocomplete="off">
                                            <label class="btn btn-outline-secondary" for="qs-years"
                                                data-mode="last_years"> Năm</label>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div style="flex:1; position:relative;">
                                <canvas id="incomeChart"
                                    style="width:100%; height:100%; display:block; position:absolute; left:0; top:0; right:0; bottom:0;"></canvas>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4" style="border-left:1px solid #e9ecef; padding-left:10px;">
                    <div id="chart-summary" class="p-3 border rounded"
                        style="height:70vh; background:#fff; display:flex; flex-direction:column;">
                        ...
                        <div
                            style="margin-top:12px; background:#fff; border-radius:6px; padding:12px; box-shadow: 0 0 0 1px rgba(0,0,0,0.02);">
...
{{-- Các script dành riêng cho trang này (Chart.js và logic AJAX) --}}
@section('scripts')
    {{-- layout2.blade.php đã tải jQuery, Bootstrap, Moment.js, v.v.
         Chúng ta chỉ cần tải thêm Chart.js --}}
        ...
        (function($) {
            ...
            function renderChart(labels, values, mode) {
                ...
                chart = new Chart(ctx, {
                    ...
                    data: {
                        labels: labels,
                        datasets: [{
                            label: 'Thu nhập (' + (mode === 'month' ? 'tháng' : (mode ===
                                'week' ? 'tuần' : '')) + ')',
                            data: values,
                                    ...
                                    callback: function(v) {
                                        ...
                                        function compact(n) {
                                            if (n >= 1000000) {
                                                ...
                                                var k = n / 1000;
                                                return (Math.round(k * 10) / 10).toString()
                                                    .replace(/\.0$/, '') + 'K';
                                            }
                                            return n.toString();
                                        }
                                ...
                                callbacks: {
                                    label: function(context) {
                                        var v = context.parsed && context.parsed.y !==
                                            undefined ? context.parsed.y : context.raw;
            ...
            function loadStats(mode, range) {
                ...
                $.get('/api/hoadon/stats', params).done(function(resp) {
                    ...
                    var chartMode = mode.indexOf('week') >= 0 ? 'week' : (mode.indexOf('year') >= 0 ?
                        'year' : 'month');
                    ...
                    $('#summary-content').html('<div>Tổng: <strong>' + fmt(total) +
                        ' đ</strong></div><div>Số mục: ' + cnt + '</div>');
                }).fail(function() {
            ...
            function renderTopRooms(data) {
                ...
                for (var i = 0; i < labels.length; i++) {
                    ...
                    html +=
                        '<div style="display:flex;align-items:center;margin-bottom:10px;font-size:13px;line-height:1.1;">';
                    html +=
                        '<div style="width:12px;height:12px;background:' + colors[i] +
                        ';margin-right:8px;border-radius:2px;flex:0 0 auto;"></div>';
            ...
            function loadTopRooms() {
                $.get('/api/datphong/top-rooms?limit=3').done(function(resp) {
                    ...
                    $('#topRoomsLegend').html('<div class="text-danger">Lỗi tải dữ liệu</div>');
                });
            }
            ...
            function renderTopServices(data) {
                ...
                for (var i = 0; i < labels.length; i++) {
                    ...
                    html += '<div style="width:10px;height:10px;background:' + colors[i] +
            ...
            function loadTopServices() {
                $.get('/api/dichvu/top-used?limit=4').done(function(resp) {
                    var data = resp && resp.data ? resp.data : [];
                    if (!data.length) {
                        $('#topServicesLegend').html(
                            '<div class="text-muted">Không có dữ liệu</div>');
                        return;
                    }
                    renderTopServices(data);
                }).fail(function() {
                    $('#topServicesLegend').html('<div class="text-danger">Lỗi tải dịch vụ</div>');
                });
            }
